...
GLCheckCollision *hit = new GLCheckCollision(); // collision detection
                              ...
                              // DO NOT ABUSE ME
{
   ...
   glDisable(GL_LIGHTING);
    for(int i=0; i<20; i++)
    {
       ...
       if(E[i].pos.x >3.5){E[i].action =E[i].WALKLEFT;E[i].speed =-0.01; E[i].pos.y = -1.2;}
       if(E[i].pos.x<-3.5){E[i].action =E[i].WALKRIGHT;E[i].speed =0.01; E[i].pos.y = -1.2;}
