...
void GLTexture::loadTexture(char* fileName)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image); //creates image and puts it in data space / storing data
    SOIL_free_image_data(image); // free up pointer with the image data / delete image holder
